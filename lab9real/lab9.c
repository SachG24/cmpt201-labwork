...
port number). Address: 127.0.0.1 Port Number: 8000
2. Is it UDP or TCP? How do you know?
TCP as it is created with SOCK_STREAM instead of SOCK_DGRAM
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? It reads from the user with STDIN and
that data is sent to the socket
4. How does the client program end? How can you tell that in the code?
when the user just enters a blank line, the num_read line = 1 so the while loop
breaks the socket is closed and it calls exit
*/
...
int main() {
  ...
  for (;;) {
    ...
    int current_id = client_id_counter++;
