...
#define MAX_FILE_SIZE 512
  ...
  size_t msg_length = read_all_bytes(message_path, message, MAX_FILE_SIZE);
  size_t sig_length = read_all_bytes(sign_path, signature, MAX_FILE_SIZE);
  ...
  if (EVP_DigestVerifyUpdate(mdctx, message, msg_length) != 1) {
    handle_error("DigestVerifyUpdate");
    EVP_MD_CTX_free(mdctx);
    return -1;
  }

  int result = EVP_DigestVerifyFinal(mdctx, signature, sig_length);
  EVP_MD_CTX_free(mdctx);
  if (result == 1) {
    return 1;
  } else if (result == 0) {
